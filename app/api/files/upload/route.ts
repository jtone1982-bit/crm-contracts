...
export async function POST(request: Request) {
  ...
  for (const file of files) {
    ...
    const { data, error } = await supabase.storage.from('candidate-files').upload(path, file)

    if (error) {
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    const { data: publicUrlData } = supabase.storage.from('candidate-files').getPublicUrl(data.path)
